class ScrollerVis {
  ...
  initVis() {
    ...
    // Identify assignment with the longest time
  ...
  step1() {
    ...
    vis.rect.transition()
        .attr('fill', d => d.difficulty=='Easy' ? 'green' : '#ccc');
  }
  ...
  step2() {
    ...
    vis.rect.transition()
        .attr('fill', d => d.difficulty=='Intermediate' ? 'yellow' : '#ccc');
  }
  step3() {
    const vis = this;
  ...
  step4() {
    ...
    // getting changed in step5() and we want to allow users to scroll up and down.
    vis.rect.transition()
        .attr('opacity', 1)
